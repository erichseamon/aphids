...
models <- cbind(model_TotAph$importance,model_TotAphNoMfc$importance,model_RelAbundMfc$importance,model_TotRp$importance,model_TotSa$importance,model_TotMd$importance,model_TotMfc$importance,model_TotSe$importance)

colnames(models) <- c("TotAph", "TotAphNoMfc", "RelAbundMfc", "TotRp", "TotSa", "TotMd", "TotMfc", "TotSe")
rownames(models) <- c("Winter Wheat", "Spring Wheat", "All Wheat", "All Cereals", "All Grasslands", "All Legumes", "Simpson's Diversity", "Cumulative Degree Days", "Cumulative Precip" )
models <- t(models)
...
model_other_refined <- subset(model_other, Var2 == "All Wheat" | Var2 == "Simpson's Diversity" | Var2 == "Cumulative Degree Days" | Var2 == "Cumulative Precip")
...
ggplot(model_other_refined, aes(x = Var2,  y = value, group = Var1, colour = Var1)) +
  geom_line(aes(color = Var1)) + theme_bw() +
  theme(axis.text.x = element_text(angle = 90)) + xlab("Cropping systems and Climate") +
  ylab("Feature Importance") +
